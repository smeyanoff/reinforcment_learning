...
class MDP:
    ...
    def get_reward(self, state, action, next_state):
        ...
        return self._rewards.get(state, {}).get(action, {}).get(
            next_state,
            0.0,
        )
        ...
        for state in transition_probs:
            ...
            for action in transition_probs[state]:
                assert isinstance(
                    ...
                    dict,
                ), "transition_probs for %s, %s should be a " \
                ...
                assert len(
                    next_state_probs,
                ) != 0, "from state %s action %s leads to no next states" % (
                    state, action,
                )
                sum_probs = sum(next_state_probs.values())
                assert abs(
                    sum_probs - 1,
                ) <= 1e-10, "next state probabilities for state %s action %s " \
...
class FrozenLakeEnv(MDP):
    ...
    def __init__(self, desc=None, map_name="4x4", slip_chance=0.2, seed=None):
        ...
        assert ''.join(desc).count(
            'S',
        ) == 1, "this implementation supports having exactly one initial state"
        assert all(
            c in "SFHG" for c in
            ''.join(desc)
        ), "all cells must be either of S, F, H or G"
        ...
        for (row, col) in states:
            ...
            for action_i in range(len(actions)):
                ...
                for movement_i in [
                    (action_i - 1) % len(actions), action_i,
...
def plot_graph(
        mdp, graph_size='10,10', s_node_size='1,5',
    ...
    for state_node in mdp._transition_probs:
        ...
        for posible_action in mdp.get_possible_actions(state_node):
            ...
            for posible_next_state in mdp.get_next_states(
                ...
                graph.edge(
                    action_node, posible_next_state,
...
def get_optimal_action_for_plot(mdp, state_values, state, gamma=0.9):
    ...
    q_values = [
        get_action_value(mdp, state_values, state, action, gamma) for
...
def plot_graph_optimal_strategy_and_state_values(mdp, state_values, gamma=0.9):
    ...
    for state_node in mdp._transition_probs:
        ...
        for action in mdp.get_possible_actions(state_node):
            if action == get_optimal_action_for_plot(
                mdp,
                state_values,
                state_node,
                gamma,
